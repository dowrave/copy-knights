...
public class OperatorLevelData : ScriptableObject
{
    ...
    public int GetExpRequirement(OperatorElitePhase phase, int level)
    {   
        // 최대 레벨이거나 잘못된 레벨이면 0 반환
        if (phase == OperatorElitePhase.Elite0 && level >= ELITE0_MAX_LEVEL) return 0;
        if (phase == OperatorElitePhase.Elite1 && level >= ELITE1_MAX_LEVEL) return 0;
        if (level < 1) return 0;
        ...
        return phase switch
        {
            OperatorElitePhase.Elite0 when arrayIndex < baseLevelUpExpRequirements.Count
                => baseLevelUpExpRequirements[arrayIndex],
            OperatorElitePhase.Elite1 when arrayIndex < phase1LevelUpExpRequirements.Count
